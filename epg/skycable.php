<?php
  ...
  foreach ($epgs["events"] as $programme) {
    ...
    $xml .= "  <programme start=\"" . date("YmdHis", strtotime($programme["start"])) . " +0800\" stop=\"" . date("YmdHis", strtotime($programme["end"])) . " +0800\" channel=\"" . preg_replace('/[^A-Za-z0-9\-]/', '', $epgs["location"][array_search($programme["location"], array_column($epgs["location"], "id"))]["name"]) . "\">\n";
